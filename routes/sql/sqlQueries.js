...
const allBranchWithStatus = 'WITH restaurantStatus AS ( SELECT rid, bid, COUNT(tid) AS num FROM freetables GROUP BY rid, bid ) SELECT rname, location, cuisineType, CASE WHEN num > 0 THEN \'AVAILABLE\' WHEN num = 0 THEN \'FULL\' END AS status FROM (advertises NATURAL JOIN restaurants NATURAL JOIN branches) R LEFT JOIN restaurantStatus S ON R.rid = S.rid AND R.bid = S.bid WHERE EXISTS ( SELECT 1 FROM freetables F WHERE F.rid = R.rid AND F.bid = R.bid AND F.availableSince < $1 );'
